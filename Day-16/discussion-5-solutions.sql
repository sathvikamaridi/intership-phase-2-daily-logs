...
select c.customerNumber,c.customerName from customers c where exists(select o.orderNumber,sum(od.priceEach*od.quantityOrdered) as total from orders o,orderdetails od where o.orderNumber=od.orderNumber and 
customerNumber=c.customerNumber group by orderNumber having sum(priceEach*quantityOrdered)>60000);
...
select customerNumber,floor(sum(quantityOrdered*priceEach)) as sales,(case when (sum(quantityOrdered*priceEach)) > 100000 then 'Platinum' when (sum(quantityOrdered*priceEach)) between 10000 and 100000 then 'Gold' when (sum(quantityOrdered*priceEach)) < 10000 then 'Silver' end) 
as customerGroup from orderdetails inner join orders using(orderNumber) where year(shippedDate)=2003 group by customerNumber order by customerNumber;

-- 10.Use the previous query as the derived table to know the number of customers in each group: platinum, gold, and silver.
select customerGroup,count(customerGroup) as groupCount from (select customerNumber,floor(sum(quantityOrdered*priceEach)) as sales,(case when (sum(quantityOrdered*priceEach)) > 100000 then 'Platinum' when (sum(quantityOrdered*priceEach)) between 10000 and 100000 then 'Gold' when (sum(quantityOrdered*priceEach)) < 10000 then 'Silver' end) 
as customerGroup from orderdetails inner join orders using(orderNumber) where year(shippedDate)=2003 group by customerNumber order by customerNumber) as derived_table1 group by customerGroup;